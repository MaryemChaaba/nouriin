  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        const { accessToken, refreshToken, ...userData } = res.data as {
          accessToken: string;
          refreshToken: string;
          [key: string]: any;
        };
        ...
        toast.error(
          (res.error?.message as string) || "Invalid email or password.",
        );
        setIsLoading(false);
      }
    } catch {
  ...
  const handleForgot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(
          (res.error?.message as string) || "Could not send reset link.",
        );
      }
    } catch {
      toast.error("Something went wrong. Please try again.");
